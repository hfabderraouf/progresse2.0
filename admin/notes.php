...
if ($mode === 'edit') {
    ...
    $r = $conn->execute_query("SELECT * FROM modules WHERE id = ?", [$selectedModuleId]);
    $module = $r->fetch_assoc();

    if (!$student || !$module) {
        header('Location: /admin/notes.php?notfound');
        die();
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!count($errors)) {
            ...
            header('Location: /admin/notes.php?' . http_build_query($_GET));
            die();
        }
    }
    ...
    $r = $conn->execute_query("SELECT * FROM modules ORDER BY id ASC");
...
<body class="container">
    ...
    <div class="main">
        <div class="navbar">
            <?php if ($mode === "edit"): ?>
                <a class="nav-link" href="/admin/notes.php">◀ Retour</a>
            <?php endif; ?>
